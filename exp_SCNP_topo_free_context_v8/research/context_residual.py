...
class PretrainedUNet(nn.Module):
    def __init__(self,method='dino_context'):
        super().__init__();assert method in ('local_refine','dino_context');self.method=method
        self.base=LocalUNet();warm=ROOT/WARM_REL
        assert hashlib.sha256(warm.read_bytes()).hexdigest()==WARM_SHA
        checkpoint=torch.load(warm,map_location='cpu',weights_only=False);assert checkpoint['step']==1200
